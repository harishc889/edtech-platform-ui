    ...
    <article className="group flex flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-md transition duration-300 hover:-translate-y-0.5 hover:border-sky-200/80 hover:shadow-xl hover:shadow-sky-500/10">
      ...
      >
        ...
        <div className="relative flex h-full flex-col justify-end p-6 sm:p-8">
          {isEnrolled ? (
            <span className="mb-3 inline-flex w-fit rounded-full bg-emerald-500/90 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-white shadow-sm">
              Already Enrolled
            </span>
          ) : null}
          <h3
            className={`font-display text-2xl font-extrabold leading-[1.15] tracking-tight text-white sm:text-[1.65rem] lg:text-3xl ${hasCoverImage ? "drop-shadow-[0_2px_12px_rgb(0_0_0/0.45)]" : "drop-shadow-sm"
              }`}
          >
            {title}
          </h3>
      ...
      <div className="flex flex-1 flex-col p-6 sm:p-8">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <Link
            ...
            className={`inline-flex flex-1 items-center justify-center rounded-full px-6 py-3.5 text-sm font-bold transition ${
              isEnrolled
                ? "pointer-events-none bg-emerald-100 text-emerald-800"
                : "bg-gradient-to-r from-sky-600 to-blue-600 text-white shadow-lg shadow-sky-500/20 hover:from-sky-500 hover:to-blue-500"
            }`}
          >
